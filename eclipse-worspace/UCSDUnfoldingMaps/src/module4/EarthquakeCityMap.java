...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(0, 0, 0);
		text("City Marker", 70, 110);
		text("Land Quake", 70, 130);
		text("Ocean Quake", 70, 150);
		text("Size ~ Magnitude", 50, 170);
		text("Shallow", 70, 210);
		text("Intermediate", 70, 230);
		text("Deep", 70, 250);
		text("Past Day", 70, 270);
	}
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> countryAndQuakes = new HashMap<String, Integer>();
		for (Marker cm : countryMarkers) {
			String nameCountry = (String)cm.getProperty("name");
			int quakeCounter = 0;
			for (Marker qm : quakeMarkers) {
				if (qm instanceof LandQuakeMarker) {
					String countryQuake = (String)qm.getProperty("country");
					if (nameCountry.equals(countryQuake)) {
						quakeCounter += 1;
					}
				} 
			}
			countryAndQuakes.put(nameCountry, quakeCounter);
		}
		// TreeMap to store values of HashMap 
        TreeMap<String, Integer> sorted = new TreeMap<String, Integer>();
        sorted.putAll(countryAndQuakes);
		for (Entry<String, Integer> key : sorted.entrySet()) {
			if (key.getValue() > 0) {
				System.out.print(key.getKey());
				System.out.println(" had " + key.getValue() + " quakes.");
			}
		}
		System.out.println("Ocean quakes " + quakeOceanCounter);
	}
