...
public class ParseFeed {
	...
	public static HashMap<String, Float> loadCOVIDFromCSV(PApplet p, String fileName) {
		// HashMap key: country ID and  data: COVID increase rate last 15 days
		HashMap<String, Float> covidMap = new HashMap<String, Float>();

		// get lines of csv file
		String[] rows = p.loadStrings(fileName);
		
		// Reads country name and population density value from CSV row
		for (int i = 61; i < rows.length - 1; i++) {
		//for (int i = 61; i < 100; i++) {	
			// split row by commas not in quotations
			String[] columnsCurr = rows[i].split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
			String[] columnsCurrPlus = rows[i+1].split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
			float firstHalf = 0;
			float averageFirstHalf = 0;
			for (int j = 60; j >= 30; j--) {
				String[] columnsCalc = rows[i-j].split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
				firstHalf += Float.parseFloat(columnsCalc[4]);
				//System.out.println("averageFirstFive " + averageFirstFive);
				//System.out.println("Float.parseFloat(columnsCalc[4]) " + Float.parseFloat(columnsCalc[4]));
				averageFirstHalf = firstHalf/30;
			}
			
			float lastHalf = 0;
			float averagelastHalf = 0;
			for (int k = 30; k >= 0; k--) {
				String[] columnsCalc = rows[i-k].split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
				//System.out.println("averagelastFive " + averagelastFive);
				lastHalf += Float.parseFloat(columnsCalc[4]);
				averagelastHalf = lastHalf/30;
			}
			
			if (!columnsCurr[2].equals(columnsCurrPlus[2])) {					
				
				System.out.println(columnsCurr[2].trim());
				covidMap.put(columnsCurr[2].trim(), (averagelastHalf/averageFirstHalf)*100-100);
			}
		}
		return covidMap;
