...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		{
		    if (i == charactors.length-1 && Character.toLowerCase(charactors[i]) == 'e' 
		    		&& newSyllable && numberSy > 0) {
                numberSy--;
            }
		    if (newSyllable && vowels.indexOf(Character.toLowerCase(charactors[i])) >= 0) {
				newSyllable = false;
				numberSy++;
			}
			else if (vowels.indexOf(Character.toLowerCase(charactors[i])) < 0) {
				newSyllable = true;
			}
		}
		return numberSy;	
